...
pub trait BooleanOps: Sized {
    ...
    fn boolean_op(&self, other: &Self, op: OpType) -> Result<MultiPolygon<Self::Scalar>, ()>;
    fn intersection(&self, other: &Self) -> Result<MultiPolygon<Self::Scalar>, ()> {
        self.boolean_op(other, OpType::Intersection)
    }
    fn union(&self, other: &Self) -> Result<MultiPolygon<Self::Scalar>,()> {
        self.boolean_op(other, OpType::Union)
    }
    fn xor(&self, other: &Self) -> Result<MultiPolygon<Self::Scalar>, ()> {
        self.boolean_op(other, OpType::Xor)
    }
    fn difference(&self, other: &Self) -> Result<MultiPolygon<Self::Scalar>, ()> {
